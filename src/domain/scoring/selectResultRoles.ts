...
function buildTiedResult(scoreable: CategoryScore[], config: ScoringConfig): ResultSelection {
  ...
  if (band === "Constraint") {
    ...
    tieState = "midrange-tied";
    tieMessage =
      "Every dimension we could measure landed at the same level, so this is a genuinely balanced profile rather than one clear standout or weak spot. The priorities below are sequenced using our standard framework order.";
  }
...
function buildRankedResult(scoreable: CategoryScore[], config: ScoringConfig): ResultSelection {
  ...
  const roles: ResultRoles = {
    strength: strengthDim ? buildInsight(strengthDim, config, "strength") : null,
    constraint: constraintDim ? buildInsight(constraintDim, config, "constraint") : null,
    opportunity: opportunityDim ? buildInsight(opportunityDim, config, "opportunity") : null,
    tieState: "none",
    tieMessage: null,
  };
